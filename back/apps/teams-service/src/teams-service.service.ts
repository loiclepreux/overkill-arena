...
export class TeamsServiceService {
  ...
  async create(data: {
    ...
    if (existing) {
      throw new RpcException({
        statusCode: 409,
        message: 'Ce nom ou tag est déjà pris',
      });
    }
  ...
  async getAll() {
    ...
    const winMap = new Map(
      matchWins.map((w) => [w.winnerId as string, w._count.id]),
    );
  ...
  async getJoinRequests(data: { teamId: string; requesterId: string }) {
    ...
    if (team.captainId !== data.requesterId) {
      throw new RpcException({
        statusCode: 403,
        message: 'Accès réservé au capitaine',
      });
    }
  ...
  async respondJoinRequest(data: {
    ...
    if (data.accepted) {
      ...
      if (alreadyMember) {
        ...
        throw new RpcException({
          statusCode: 409,
          message: 'Cet utilisateur est déjà dans une équipe',
        });
      }
  ...
  async leave(data: { teamId: string; userId: string }) {
    ...
    if (team.captainId === data.userId) {
      throw new RpcException({
        statusCode: 400,
        message: "Le capitaine ne peut pas quitter l'équipe, supprimez-la",
      });
    }
  ...
  async kickMember(data: {
    ...
    if (team.captainId !== data.requesterId) {
      throw new RpcException({
        statusCode: 403,
        message: 'Seul le capitaine peut exclure un membre',
      });
    }
    if (data.targetUserId === data.requesterId) {
      throw new RpcException({
        statusCode: 400,
        message: "Le capitaine ne peut pas s'exclure lui-même",
      });
    }
    const membership = await this.prisma.teamMember.findFirst({
